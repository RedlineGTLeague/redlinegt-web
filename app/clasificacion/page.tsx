      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid gap-8 lg:grid-cols-2">
                ...
                {drivers.map((driver) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      <div className="h-4 w-1 rounded-full" style={{ backgroundColor: driver.teamColor }} />
                      <span className="text-sm text-muted-foreground">{driver.team}</span>
                    </div>
                    <span className="text-right font-bold text-foreground">{driver.points}</span>
                  </div>
                ))}
              </div>
            </CardContent>
          ...
          <Card className="border-border bg-card">
            ...
            <CardContent>
              {/* Table Header */}
              <div className="mb-3 grid grid-cols-[3rem_1fr_5rem] gap-4 border-b border-border px-3 pb-3 text-sm font-medium uppercase tracking-wider text-muted-foreground">
                <span>Pos</span>
                <span>Equipo</span>
                <span className="text-right">Puntos</span>
              </div>
              ...
              <div className="space-y-2">
                {teams.map((team) => (
                  <div
                    key={team.position}
                    className="grid grid-cols-[3rem_1fr_5rem] items-center gap-4 rounded-lg bg-secondary/50 px-3 py-3 transition-colors hover:bg-secondary"
                  >
                    <span className={`flex h-8 w-8 items-center justify-center rounded-full text-sm font-bold ${
                    ...
                    <div className="flex items-center gap-3">
                      <div className="h-6 w-1.5 rounded-full" style={{ backgroundColor: team.color }} />
                  ...
                  {teams.slice(0, -1).map((team, index) => (
                    <div key={team.name} className="flex items-center justify-between text-sm">
                      <span className="text-muted-foreground">
                        {team.name} vs {teams[index + 1].name}
                      </span>
